...
def analysis_correlation_by_tourspot(result_files):
    with open(result_files['tourspot_visitor'], 'r', encoding='utf-8') as infile:
    ...
    for tourspot in tourspots:
        ...
        for filename in result_files['foreign_visitor']:
            with open(filename, 'r', encoding='utf-8') as infile:
                json_data = json.loads(infile.read())

            foreign_visitor_table = pd.DataFrame(json_data, columns=['country_name', 'date', 'visit_count'])
            country_name = foreign_visitor_table['country_name'].unique().item(0)

            foreign_visitor_table = foreign_visitor_table.set_index('date')
            merge_table = pd.merge(temp_tourspot_table, foreign_visitor_table, left_index=True, right_index=True)
...
def analysis_correlation(result_files):
    with open(result_files['tourspot_visitor'], 'r', encoding='utf-8') as infile:
        json_data = json.loads(infile.read())
